...
extern sfr sbit WAKE;               // WAKE pin
extern sfr sbit WAKE_Direction;     // WAKE pin direction
extern short int DATA_TX[]; // used for test send function
...
void init_ZIGBEE_basic() {
  ...
  write_ZIGBEE_long(TESTMODE, 0x0F);  // Optimized RSSI (default) and Set to use external LNA and PA on high power module
}
...
void init_ZIGBEE_nonbeacon() {
  ...
  set_channel(ZIGBEE_CHANNEL);     // Tested at 25 - now using  20 - 15, 20 ,25,26 are non wifi overlapping
  RF_reset();
}

void send_test_message(uint8_t cmd)
{
     // Send out the command as a quick message
     // sent as a broadcast message
     DATA_TX[0] = cmd; // Command for Bind with Flash
     write_TX_normal_FIFO();          // Transmiting
     //LATD = DATA_TX[0];               // Showing value that is sent on PORTD
     //DATA_TX[0]++;                    // Incremeting value
     //Delay_ms(1000);
}

 void send_broadcast_message(uint8_t cmd)
{
     // Send out the command as a quick message
     // sent as a broadcast message
     DATA_TX[0] = cmd; // Command for Bind with Flash
